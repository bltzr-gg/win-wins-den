...
import { Rocket, Clock, Wallet, ArrowRight, Shield, Users, Calendar, Mail } from "lucide-react";
      ...
      {units.map((u, i) => (
        <div key={u.label} className="flex items-center gap-3">
          <div className="text-center">
            <div className="w-16 h-16 rounded-xl bg-secondary/40 border border-border/40 flex items-center justify-center">
              <span className="font-display text-2xl text-muted-foreground">{u.value}</span>
            </div>
            <p className="text-[9px] text-muted-foreground uppercase tracking-wider mt-1.5">{u.label}</p>
          </div>
          {i < units.length - 1 && <span className="font-display text-xl text-muted-foreground/30 mb-5">:</span>}
        </div>
      ))}
    </div>
  );
...
function InfoBlock({ icon: Icon, title, description, delay }: {
  ...
  return (
    <motion.div
      className="rounded-2xl border border-border/40 bg-card p-6 space-y-3"
  ...
  return (
    <div className="space-y-10">
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-br from-[hsl(0_20%_6%)] via-background to-[hsl(240_10%_5%)]" />
        <div className="absolute top-1/3 left-1/4 w-[500px] h-[300px] bg-[hsl(0_60%_25%/0.04)] rounded-full blur-[120px] pointer-events-none" />
        <div className="absolute bottom-0 right-1/4 w-[400px] h-[200px] bg-[hsl(260_40%_25%/0.03)] rounded-full blur-[100px] pointer-events-none" />
        <div className="absolute inset-0 metallic-sheen pointer-events-none" />

        <div className="relative z-10 px-8 py-16 lg:py-24 text-center border border-border/20 rounded-2xl space-y-8">
          {/* Coming Soon badge */}
          <motion.div
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-primary/8 border border-primary/15"
          ...
          <div className="space-y-4 max-w-2xl mx-auto">
            <h1 className="font-display text-4xl lg:text-5xl tracking-wide leading-tight">
              COMMUNITY TOKEN<br />
          ...
          <div className="space-y-3">
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">TGE: Coming Soon</p>
          ...
          <motion.button
            ...
            style={{ boxShadow: "0 0 30px hsl(0 84% 40% / 0.15)" }}
          ...
          <p className="text-[10px] text-muted-foreground/60">No commitments. Be the first to know.</p>
        </div>
      </motion.div>
      ...
      {/* Trust strip */}
      <motion.div
        className="rounded-xl border border-border/30 bg-card p-5 flex flex-col sm:flex-row items-center justify-center gap-6 text-center"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
